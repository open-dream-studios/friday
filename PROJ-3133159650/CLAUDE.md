...
Drawn from TCR's own contract documents (Fairground Lane proposal + take-off). Delete anything that doesn't hold.

- **Out-of-core jobs cost drive time.** Jobs outside the Tri-Cities core (e.g. Mountain City, Johnson County — 45-60 min from Johnson City) get shorter effective on-site days: crew travel eats hours, inspections and deliveries take longer to arrange, and forgotten-material runs are expensive. Plan fewer, longer site visits per trade and pad inspection scheduling.
- **Base-scope exclusions (from TCR's own proposal language):** hidden rot, termite damage, inadequate existing framing, concealed utilities, rock excavation, groundwater intrusion, unsuitable soils, and code-mandated upgrades not visible at estimate — all change orders when encountered. Matching existing finishes (siding, roofing, masonry) is best-effort; age and discontinued products can prevent exact matches.
- **Field measurements govern over drawings.** Contractor verifies field measurements, elevations, drainage flow, tie-in conditions, and utility locations before construction. Rendering packages are visualization only — "colors/materials may not reflect exact manufacturer products."
- **Allowance pricing sanity.** Client-selected materials run as retail allowances; estimates built on national-average retail data are confirmed against local Tri-Cities supplier pricing before any order is placed (TCR's own take-off instruction).
